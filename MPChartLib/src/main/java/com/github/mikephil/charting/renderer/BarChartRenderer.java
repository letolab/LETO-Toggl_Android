...
public class BarChartRenderer extends BarLineScatterCandleBubbleRenderer {
    ...
    protected void drawDataSet(Canvas c, IBarDataSet dataSet, int index) {
        ...
        // if multiple colors
        if (dataSet.getColors().size() > 1 || dataSet.useStackedColors()) {
            ...
            for (int j = 0; j < buffer.size(); j += 4) {
                ...
                if (!dataSet.useStackedColors()) {
                    ...
                    int tempSize = 0;
                    List<List<Integer>> stackedCols = ((BarDataSet)dataSet).stackedColors;
                    for (int colVal = 0; colVal< stackedCols.size(); colVal++){
                        if (j < (stackedCols.get(colVal).size()+tempSize)*4){
//                            Log.d("Value of j", String.format("%d - %d",j,colVal));
                            color = stackedCols.get(colVal).get(j/4-tempSize);
                            if (j+4 >= (stackedCols.get(colVal).size()+tempSize)*4){
                                isLast=true;
                            }
                            break;
                        }
                        tempSize += stackedCols.get(colVal).size();
                    }
                    mRenderPaint.setColor(color);
                }
                if (isLast){
                    c.drawPath(RoundedRect(buffer.buffer[j], buffer.buffer[j + 1], buffer.buffer[j + 2],
                            buffer.buffer[j + 3],10,10,true), mRenderPaint);
                }else{
    ...
    private Path RoundedRect(float left, float top, float right, float bottom, float rx, float ry, boolean conformToOriginalPost) {
        Path path = new Path();
        if (rx < 0) rx = 0;
        if (ry < 0) ry = 0;
                        ...
                        if (vals == null) {
                            ...
                            for (int k = 0; k < transformed.length; k += 2) {
                                ...
                                if (!dataSet.useStackedColors()) {
                                    ...
                                    drawValue(c, dataSet.getValueFormatter(), vals[k / 2], entry, i, x, y, Color.RED);
                                }
                            }
                        }
